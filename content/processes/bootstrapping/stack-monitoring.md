...
* `Config variables` can be configured by creating a `monitoring.sh` file in the root folder and export the stack specific variables from there. It will reduce the chances of errors in configuration because now few CI/CD variables needs to be changed for deployment. Example of config file is given below:
